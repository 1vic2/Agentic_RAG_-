<template>
  <div class="content kb-page" style="padding:0;max-width:100%">
    <div style="display:flex;flex:1;gap:16px;padding:24px;height:100%">
      ...
      <div v-if="!s" class="kb-empty" style="flex:1;display:flex;align-items:center;justify-content:center;color:var(--text-3);font-size:14px;gap:12px;flex-direction:column">
      ...
      <div v-else style="flex:1;display:flex;flex-direction:column;gap:16px;min-width:0">
        <div class="upload-dropzone" style="padding:40px 24px;border:2px dashed rgba(255,255,255,.06);border-radius:20px;text-align:center;cursor:pointer;transition:all .3s;background:rgba(15,23,42,.3);backdrop-filter:blur(8px)" @click="fi?.click()" @drop.prevent="onDrop" @dragover.prevent>
          <input ref="fi" type="file" multiple accept=".pdf,.docx,.doc,.pptx,.txt,.md,.csv,.xlsx,.xls,.html,.htm,.xml,.json,.rtf,.eml,.msg,.epub,.png,.jpg,.jpeg,.tiff,.bmp,.webp,.odt,.odp,.ods" style="display:none" @change="onInput" />
        ...
        <div v-if="fs.length" style="display:flex;flex-direction:column;gap:4px">
          <div v-for="f in fs" :key="f.filename" class="document-row" style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:rgba(15,23,42,.4);border:1px solid rgba(255,255,255,.04);border-radius:12px;font-size:13px;transition:border-color .2s">
            ...
            <button @click="dd(f.filename)" class="document-delete" style="color:var(--text-3);padding:2px;transition:color .2s">
...
const up = async (f: FileList) => {
  if (!s.value) return
  const fm = new FormData()
  Array.from(f).forEach(x => fm.append('files', x))
  try {
    const response = await fetch(`/api/knowledge-bases/${s.value.id}/upload`, { method: 'POST', body: fm })
    const data = await response.json()
    if (!response.ok) msg.value = data.detail || '上传失败'
    else {
      const parts = [`已上传 ${data.saved?.length || 0} 个文件`]
      if (data.rejected?.length) parts.push(`拒绝: ${data.rejected.map((x:any) => `${x.filename}（${x.reason}）`).join(', ')}`)
      if (data.warnings?.length) parts.push(data.warnings.join(', '))
      msg.value = parts.join('；')
      await ld()
    }
  } catch { msg.value = '上传请求失败' }
